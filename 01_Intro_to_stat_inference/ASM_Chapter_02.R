...
NLL <- function(param, y, x) {
  alpha <- param[1]        # Intercept
  beta <- param[2]         # Slope
  lambda <- exp(alpha + beta * x)   # Linear model, diferent expected value for each year (x, center in 0, scaled from -16 to 16)
  LLi <- dpois(y, lambda, log=TRUE) # LL contribution for each single observation i
...
VC <- solve(out$hessian)       # Get variance-covariance matrix
  #see negative correlation between the two parameters
