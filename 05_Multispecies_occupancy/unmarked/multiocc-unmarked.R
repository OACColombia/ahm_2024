...
#' 
## -----------------------------------------------------------------------------
ylist <- MesoCarnivores[c(2:3)] #selecting the coyote and redfox
...
head(umf) #only shows a species, but we can see the structure
...
#' 
## -----------------------------------------------------------------------------
umf@fDesign #slot to see the math of \psi
...
#' 
## -----------------------------------------------------------------------------
#Formulas in quotations
stateformulas <- c("~scale(HDens_5km)", #covariate for f1
                   "~scale(HDens_5km)", #covariate for f2
                   "~1") #covariate for f3
stateformulas
...
#' 
## -----------------------------------------------------------------------------
detformulas <- c("~1", #sp1 in the order, in the example coyote
                 "~1") #sp2 in the order, in the example redfox
detformulas
...
#' 
## -----------------------------------------------------------------------------
#example with house densities covariates
...
## -----------------------------------------------------------------------------
mod_hdens
#coyote is more common (make sense, more coyote detections)
#intercept for interation is big and positive, the species tend to occur in same sizes
...
#Change state formula and detection formula
...
#distance almost not change intercept interaction term (almost 0)

#detection
#if you ar in a trail, it is more probable to detect both species!
...
#a list with four columns (the psi)
lapply(occ_prob, head)
#if you sum each row, they should sum 1
...
#just for site 1 on a single plot
...
#' 
#' ### Co-occurrence (joint) probability
#' 
...
## -----------------------------------------------------------------------------
occ_hdens_coypres <- predict(mod_hdens, 
                             type="state", 
                             species="redfox", 
                             cond="coyote", #the conditional
                             newdata=nd)
occ_hdens_coypres$Coyote_status <- "Present"

occ_hdens_coyabs <- predict(mod_hdens, 
                            type="state", 
                            species="redfox", 
                            cond="-coyote", 
                            newdata=nd)
...
#' 
## -----------------------------------------------------------------------------
stateformulas_0 <- c("~scale(HDens_5km)", "~scale(HDens_5km)", 
                     "0") #forcing the third order interaction as 0
...
stateformulas <- c("~scale(HDens_5km)", 
                   "~scale(HDens_5km)") #not include third way, because we dont want it
mod_noint <- occuMulti(detformulas=detformulas, 
                       stateformulas=stateformulas, 
                       data=umf, 
                       maxOrder=1) #maxOrde1 means no interactions!
mod_noint
...
f1 <- 0.2 #species 1 is a little bit common than 2
f2 <- 0.1
f12 <- 0.5 # <- EFFECT SIZE OF INTERACTION, big!!!
...
J <- 5   # occasions
y <- matrix(NA, M, J)
temp <- unmarkedFrameOccuMulti(list(y,y)) #describe the structure of the data, two species without information
head(temp)
