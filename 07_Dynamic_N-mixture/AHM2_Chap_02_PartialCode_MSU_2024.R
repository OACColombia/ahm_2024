...
# Code from Github repo from our late friend Mike Meredith


# ----------------------------------------------------------------------
#
#   Modified some from the original for the AHM course at MSU, July 2024
#
# ----------------------------------------------------------------------
...
simDM0 <- function(nsites = 50, nsurveys = 3, nyears = 5, lambda = 4,
  ...
  for(t in 1:(nyears-1)) { # State dynamics
    ...
    R[,t] <- rpois(nsites, gamma) # Recruitment process (see absoulte recluitment gamma)
    N[,t+1] <- S[,t] + R[,t]
  }
...
#The function returns:
  #the initial parameters (nsites, nsurveys, nyears, lambda, phi, gamma, p)
  #N estimates
  #S estimates
  #R estimates
  #y a 3D array
...
cat(file = "DM1.txt","
model {
  # Priors for the basic structure parameters
  ...
  # Likelihood - exactly to the model in algebra and R code when simulate
  for(i in 1:nsites){
    ...
    for(t in 1:(nyears-1)){
      ...
      # R[i,t+1] ~ dpois(gamma)        # 'absolute' recruitment = 'constant' in unmarked
      R[i,t+1] ~ dpois(N[i,t] * gamma) # per-capita recruitment = 'autoreg' in unmarked
...
# Initial values - to avoid contradictions
...
#Gamma is different because we used the per-capita recruitment. If we unhash the absolute
...
# Call JAGS (ART 2 min), check convergence and summarize posteriors
out1 <- jags(bdata, inits, params, "DM2.txt", n.adapt = na, n.chains = nc,
             n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)
